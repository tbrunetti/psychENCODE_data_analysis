	...
	def run_pipeline(self, pipeline_args, pipeline_config):
		...
		for bam in os.listdir(pipeline_args['b']):
			if bam[-4:]=='.bam':
				...
				f=open(pathToPicardOut+'filteredBAMs.'+str(bam[:-4])+'.rnaSeqMetrics.txt')
				line=f.readlines()
				header=line[6].split('\t')
